...
public class FrmReporteDirectorConsulta extends JFrame implements ActionListener {
	...
	protected void do_btnFiltrar_actionPerformed(ActionEvent arg0) {
		//obtengo los datos de las cajas de texto
		String fecIni = txtInicio.getText().trim();
		String fecFin = txtFin.getText().trim();
	
		//instnacio a mi modelDirector que contiene el metodo para consultar
		DirectorModel model = new DirectorModel();
		//creo un list de tipo director 
		List<Director> lstData = null;
		// si al dar click en el boton 
		//las casillas estan vacias entonces
		if(fecIni.equals("") || fecFin.equals("")){
			//se llama al metodo que contiene la lista completa
			//y se la guarda en el list de tipo director que creamos
			lstData = model.listaDirector();
		}
		//si las casillas contienen datos entonces 
		else{
			//llamaos al metodo consutlar y lo guardamos en el list de tipo director 
